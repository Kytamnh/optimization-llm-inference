...
RES_DIR_PREFIX = os.environ.get('SCOOT_RES_DIR_PREFIX', 'scoot')
...
def _result_matches_combination(act_result, combination):
    return combination == (int(act_result['tp']),
...
def find_matching_result(res_dir_path, combination, started_at):
    result_files = []
    for root, _, files in os.walk(res_dir_path):
        for name in files:
            if name.startswith("vllm") and name.endswith(".json"):
                file_path = os.path.join(root, name)
                if os.path.getmtime(file_path) >= started_at:
    ...
    for file_path in result_files:
        try:
            ...
            logging.warning(f"Failed to read benchmark result {file_path}: {traceback.format_exc()}")
            continue

        if _result_matches_combination(act_result, combination):
            return act_result

    logging.warning(
        f"No matching benchmark result found in {res_dir_path} for {combination}. "
        f"Checked {len(result_files)} result files generated after benchmark start.")
    return None
...
def obj(rec: pd.DataFrame, gpu_nums, res_dir_path, args, min_world_size: int = 1) -> Union[None, np.ndarray]:
    ...
    act_result = find_matching_result(res_dir_path, combination, benchmark_started_at)
