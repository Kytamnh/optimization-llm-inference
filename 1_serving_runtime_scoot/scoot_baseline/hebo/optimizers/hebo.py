...
from hebo.optimizers.util import update_max_num_batched_tokens
...
class HEBO(AbstractOptimizer):
    ...
    def suggest(self, n_suggestions=1, fix_input = None):
        ...
        if self.X.shape[0] < self.rand_sample:
            ...
            sample = sample.apply(update_max_num_batched_tokens, axis=1)
            sample.loc[sample['max_num_batched_tokens'] < sample['max_num_seqs'], 'max_num_seqs'] = sample['max_num_batched_tokens']
            return sample
        else:
            ...
            acq = self.acq_cls(model, space= self.space, num_hard_constr = 2, best_y = py_best, kappa = kappa) # LCB < py_best
            mu  = Mean(model)
            ...
            while rec.shape[0] < n_suggestions:
                ...
                rand_rec = rand_rec.apply(update_max_num_batched_tokens, axis=1)
                rand_rec.loc[sample['max_num_batched_tokens'] < sample['max_num_seqs'], 'max_num_seqs'] = sample['max_num_batched_tokens']
